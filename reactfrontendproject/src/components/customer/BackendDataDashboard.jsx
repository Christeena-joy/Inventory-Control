...
export default function BackendDataDashboard() {
    ...
    const columns = [
        { field: 'cname', headerName: 'Name', width: 180, editable: true },
        { field: 'email', headerName: 'Email', width: 180, editable: true },
        { field: 'phoneNo', headerName: 'Phone', width: 180, editable: true },
    ...
    const fetchData = async () => {
        try {
            const response = await axios.get('http://localhost:8080/user/customers',{
    ...
    const validateForm = (data) => {
        return data.cname && data.email && data.phoneNo && data.address;
    };
    ...
    const handleAddRecordSubmit = async () => {
        if (validateForm(newRecordData)) {
            try {
                ...
                await axios.post('http://localhost:8080/user/customers', newRecordData, { headers });
                setOpen(false);
                setNewRecordData({}); // Clear the new record data after submission
    ...
    const handleDelete = async (row) => {
        try {
            const headers = getHeader();
            await axios.delete(`http://localhost:8080/user/customers/${row.customerId}`,{headers});
    ...
    const handleFormSubmit = async () => {
        ...
        if (validateForm(updatedFields)) {
            try {
                const headers = getHeader();
                const updatedRow = { ...selectedRow, ...updatedData }; // Merge selectedRow and updatedData
                console.log("Updated data to be sent to the backend:", updatedRow);
                await axios.put(`http://localhost:8080/user/customers/${selectedRow.customerId}`, updatedRow, { headers });
                setOpen(false);
    ...
    return (
        <Box m={2} sx={{
            ...
            sx={{marginBottom:"10px"}}>Add Customer</Button>
            <DataGrid rows={rows} columns={columns} />
            <Dialog open={open} onClose={handleClose}>
                <DialogTitle>{isAddForm ? 'Add Customer' : 'Update Customer'}</DialogTitle>
                <DialogContent>
                    <TextField
                        autoFocus
                        margin="dense"
                    ...
                    <TextField
                        ...
                        type="text"
                        fullWidth
                        defaultValue={isAddForm ? newRecordData.phoneNo : selectedRow?.phoneNo}
                        onChange={handleInputChange}
                    />
                    <TextField
                        margin="dense"
