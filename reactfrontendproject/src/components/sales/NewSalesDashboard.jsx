...
import CreateDetailedSale from "./saletransaction/CreateDetailedSale";
...
export default function NewSalesDashboard() {
  ...
  const [, setUpdatedData] = useState({});
  const [newRecordData, setNewRecordData] = useState({});
  const [isAddForm, setIsAddForm] = useState(false);
  const [options, setOptions] = useState([]);
  const [totalSales, setTotalSales] = useState(0);
  const [salesData, setSalesData] = useState([]);
  const navigate = useNavigate(); // Use useNavigate to control routing
      ...
      renderCell: (params) => (
        <ButtonGroup>
          <Button
            ...
            sx={{
                backgroundColor: '#3e87c5', // Custom color
                color: '#fff', // Ensure text color is visible
                '&:hover': {
                  backgroundColor: '#3e87c5', // Hover color
                },
              }}
            size="small"
    ...
    <Box m={2} sx={{
      ...
      <Button
                ...
                sx={{
                    ...
                    color: '#fff', // Ensure text color is visible
                    '&:hover': {
                      backgroundColor: '#2980b9', // Hover color
                    },
                    marginBottom:2
                  }}
                onClick={() => navigate("/saletransaction/create")} // Navigate to the new route when clicked
            >
                + Add New Sale
            </Button>
